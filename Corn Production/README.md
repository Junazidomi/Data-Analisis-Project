# Corn Production Analysis in Indonesia

## Background

Corn is one of Indonesia's most strategic and best-selling agricultural commodities. It is often exported and imported to meet the needs of the food and animal feed industries. Therefore, comprehensive analysis of historical data is needed to maintain the stability of domestic corn supplies. The analysis seeks to understand corn production trends from year to year, assess the area of production land, and identify the islands and provinces with the highest corn production levels in Indonesia. With this understanding, it is hoped that appropriate strategies and policy measures can be formulated to address supply issues and enhance national food security.

## Tools

The tools used in this project are a follows:

- Python Notebook
- Google Locker Studio
  
## Process

The Process in this project includes:

- Combine all dataset into a single dataset
- Identify the data structure such as missing value, duplicate data, type of data and create a data summary
- Perform preprocessing to improve the dataset, such as changing column data types, handling missing values and adding new columns based on available information
- Create data visualization using Notebook and compile dashboard using Google Locker Studio
  
## Dashboard Result

The result of this project were visualized using Google Locker Studio, and the dashboard screenshoot is shown below. [Dashboard](https://lookerstudio.google.com/reporting/488a4a08-5c13-4a61-ac8e-9d5dd2d4ff2a)

![Dashboard](https://raw.githubusercontent.com/Junazidomi/Data-Analisis-Project/refs/heads/main/Corn%20Production/Picture/Corn%20Analysis.jpg)

### Dashboard Analysis 1 : Total Corn Production by Year in Indonesia

![Dashboard1](https://raw.githubusercontent.com/Junazidomi/Data-Analisis-Project/refs/heads/main/Corn%20Production/Picture/Production%20by%20Year.png)

Insight:

Based on the visualisation of corn production in Indonesia each year, it can be concluded that corn production fluctuates from year to year. Corn production showed a significant increase in the 2021–2022 period, then declined in the 2022–2023 period.

### Dashboard Analysis 2 : Average Corn Productivity by Year in Indonesia

![Dashboard2](https://raw.githubusercontent.com/Junazidomi/Data-Analisis-Project/refs/heads/main/Corn%20Production/Picture/Productivity%20by%20Year.png)

Insight:

Based on corn productivity visualisation, it is known that corn productivity in Indonesia has increased consistently every year. The highest increase in productivity occurred in the 2022–2023 period, while the lowest increase occurred in the 2023–2024 period compared to other periods.

### Dashboard Analysis 3 : Average Corn Harvest Area by Year in Indonesia

![Dashboard3](https://raw.githubusercontent.com/Junazidomi/Data-Analisis-Project/refs/heads/main/Corn%20Production/Picture/Harvest%20Area%20by%20Year.png)

Insight:

Based on the visualisation of the average annual corn production area in Indonesia, it is evident that the corn production area fluctuates. In the 2022–2023 period, there was a significant increase, while in the following period, namely 2023–2024, the corn production area decreased.

### Dashboard Analysis 4 : Total Corn Production by Island

![](https://raw.githubusercontent.com/Junazidomi/Data-Analisis-Project/refs/heads/main/Corn%20Production/Picture/Production%20by%20Island.png)

Insight:



### Dashboard Analysis 5 : Average Corn Productivity by Province

![Dashboard5](https://raw.githubusercontent.com/Junazidomi/Data-Analisis-Project/refs/heads/main/Corn%20Production/Picture/Productivity%20by%20Province.png)

### Dashboard Analysis 6 : Total Corn Production by Province

![Dashboard6](https://raw.githubusercontent.com/Junazidomi/Data-Analisis-Project/refs/heads/main/Corn%20Production/Picture/Production%20by%20Province.png)
